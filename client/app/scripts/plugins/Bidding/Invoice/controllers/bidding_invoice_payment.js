...
/**
 * @ngdoc function
 * @name payperrApp.controller:BidFinalPaymentController
 * @description
 * # BidFinalPaymentController
 * Controller of the payperrApp
 */
angular.module('payperrApp.Bidding.Invoice')
    .controller('BiddingInvoicePaymentCtrl', function($rootScope, $scope, $window, countries, states, cities, $stateParams, usersAddresses, wallet, flash, $location, $filter, $state, paymentGateways, userSettings, ConstPaymentGateways, ProjectEditView, UserMeFactory, $cookies, Invoice, ClassName, PaymentOrderFactory, CouponGetStatusFactory, $uibModalStack) {
         ...
         if($rootScope.settings.SITE_ENABLED_PLUGINS.indexOf('Bidding/Invoice') > -1)
            {
                Invoice.get({
                    ...
                    if (parseInt(response.error.code) === 0) {
                        $scope.paymentValue = response.data;
                        $scope.plan_info.price_final = $scope.paymentValue.amount;
                        $scope.className = ClassName.invoice;
                        $scope.commission_value = parseInt($scope.plan_info.price_final * $rootScope.settings.PROJECT_COMMISSION_FROM_EMPLOYER_FOR_INVOICE)/100;
                    }
                });
            }
        ...
        $scope.index = function() {
            ...
            paymentGateways.get({}, function(payment_response) {
                ...
                if (payment_response.error.code === 0) {
					if (payment_response.zazpay !== undefined && payment_response.zazpay.error.code !== 1) {
					...
					} else if(response.paypalrest_enabled) {
						$scope.gateway_id = ConstPaymentGateways.PayPal;
					}
                }
                $scope.loader = false;
                if($scope.wallet_enabled === true && $scope.paypal_enabled !== true){
                    $scope.paneChanged('Wallet');
                }else if($scope.wallet_enabled === false && $scope.paypal_enabled === true){
                    $scope.paneChanged('paypal');
                }
            });
        };
        $scope.applyCoupon = function() {
            ...
            params.amount = $scope.paymentValue.amount;
            CouponGetStatusFactory.get(params, function(response) {
                ...
                if (response.error.code === 0) {
                    if ($scope.discountCoupon.data.discount_type_id == ConstDiscountType.Amount) {
                        $scope.plan_info.price_final = parseFloat($scope.paymentValue.amount) - parseFloat(response.data.discount);
                        $scope.show_discount = $filter("currency")(response.data.discount);
                    } else {
                        var discount_amt = ((response.data.discount / 100) * $scope.paymentValue.amount);
                        $scope.plan_info.price_final = $scope.paymentValue.amount - discount_amt;
        ...
        $scope.PaymentFormSubmit = function(form) {
            ...
            $scope.buyer.user_id = $scope.auth.id;
            $scope.buyer.foreign_id = $state.params.id;
            $scope.buyer.class = 'ProjectBidInvoice';
                    ...
                    if (response.error.code === 0) {
						if (response.redirect_url !== undefined) {
                            ...
                            $state.go('Bid_ProjectView', {
                                id: $scope.paymentValue.project.id,
                                slug: $scope.paymentValue.project.slug
                            });
                        } else if (response.error.code === 0) {
                            if (response.payment_response.status === 'Captured') {
                                $scope.my_user.available_wallet_amount = $scope.my_user.available_wallet_amount - parseInt(response.data.amount);
                            }
