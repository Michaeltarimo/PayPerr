    ...
    .controller('BiddingMilestonePaymentCtrl', function($rootScope, $scope, $window, countries, states, cities, $stateParams, usersAddresses, wallet, flash, $location, $filter, $state, paymentGateways, userSettings, ConstPaymentGateways, ProjectEditView, UserMeFactory, $cookies, Milestone, ClassName, PaymentOrderFactory, CouponGetStatusFactory, $uibModalStack) {
        ...
        Milestone.get({
            ...
            if (parseInt(response.error.code) === 0) {
                ...
                $scope.projectId = $scope.paymentValue.project.id;
                $scope.plan_info.price_final = $scope.paymentValue.amount;
                $scope.className = ClassName.milestone;
                  $scope.commission_value = parseInt($scope.plan_info.price_final * $rootScope.settings.PROJECT_COMMISSION_FROM_EMPLOYER_FOR_MILESTONE)/100;
            }
        });
        ...
        $scope.PaymentFormSubmit = function(form) {
            ...
            $scope.buyer.class = $scope.className;
            ...
            if (form) {
                ...
                PaymentOrderFactory.create($scope.buyer, function(response) {
                    if (response.error.code === 0) {
                        if (response.data.gateway_callback_url !== undefined) {
                            ...
                            if (response.payment_response.status === 'Captured') {
                                $scope.available_wallet_amount = $scope.my_user.available_wallet_amount - parseInt(response.data.amount);
                $scope.my_user.available_wallet_amount = $scope.available_wallet_amount - $scope.project_commision;
                            }
