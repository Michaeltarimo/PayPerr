'use strict';
angular.module('payperrApp.Bidding.Exam')
    .controller('ExamPaymentController', function($rootScope, $scope, $window, countries, states, cities, $stateParams, usersAddresses, wallet, flash, $location, $filter, $state, paymentGateways, userSettings, ConstPaymentGateways, ProjectEditView, UserMeFactory, ExamsPayment, ExamsView, $cookies, md5) {
         $scope.exam_id = JSON.parse($cookies.get("exam_id"));
        ...
        ExamsView.get({
            ...
            if (parseInt(response.error.code) === 0) {
                ...
                $scope.plan_info.price_final = $scope.examValue.fee;
            }
        ...
        $scope.applyCoupon = function() {
            ...
            params.amount = $scope.examValue.Exam.zazpay_revised_amount;
            CouponGetStatusFactory.get(params, function(response) {
                ...
                if (response.error.code === 0) {
                    if ($scope.discountCoupon.data.discount_type_id == ConstDiscountType.Amount) {
                        $scope.plan_info.price_final = parseFloat($scope.examValue.Exam.zazpay_revised_amount) - parseFloat(response.data.discount);
                        $scope.show_discount = $filter("currency")(response.data.discount);
                    } else {
                        var discount_amt = ((response.data.discount / 100) * $scope.examValue.Exam.zazpay_revised_amount);
                        $scope.plan_info.price_final = $scope.examValue.Exam.zazpay_revised_amount - discount_amt;
        ...
        $scope.ClearCoupon = function() {
            $scope.plan_info.price_final = $scope.examValue.Exam.zazpay_revised_amount;
        ...
        $scope.PaymentFormSubmit = function(form) {
            ...
            $scope.buyer.exam_id = $state.params.id;
            ...
            if (form) {
                ...
                ExamsPayment.create($scope.buyer, function(response) {
                    if (response.error.code === 0) {
                        if (response.redirect_url !== undefined) {
                            ...
                            if (response.payment_response.status === 'Captured') {
                                $scope.my_user.available_wallet_amount = $scope.my_user.available_wallet_amount - parseInt(response.data.fee_paid);
                            }
